...
export function serializeLineup(lineup: GrenadeLineup & { map: CsMap }) {
  ...
  return {
    ...
    mapEmoji: lineup.map.emoji,
    mapPremiumEmojiId: lineup.map.premiumEmojiId,
    mapButtonStyle: lineup.map.buttonStyle,
...
export function normalizeMediaItems(value: Prisma.JsonValue | null, fallback?: GrenadeMediaItem): GrenadeMediaItem[] {
  ...
  const items = rawItems.flatMap((item): GrenadeMediaItem[] => {
      ...
      return [{
        ...
        zoomStartSeconds: numberOrNull(record.zoomStartSeconds),
        zoomEndSeconds: numberOrNull(record.zoomEndSeconds),
        zoomScale: numberOrNull(record.zoomScale),
